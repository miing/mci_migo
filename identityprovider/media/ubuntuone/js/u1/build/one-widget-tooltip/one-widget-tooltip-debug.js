YUI.add('one-widget-tooltip', function (Y, NAME) {
...
var Tooltip = Y.Base.create("tooltip", Y.Widget, [Y.WidgetPosition, Y.WidgetPositionAlign, Y.WidgetStack], {
    ...
    bindUI : function() {
        this.after("compassChange", this._onSetCompass);
    ...
    syncUI : function() {
        ...
        this.get("boundingBox").addClass(Y.ClassNameManager.getClassName('tooltip', 'position', this.get("compass")));
    },
    ...
    _onSetCompass: function(e) {
        var bb = this.get("boundingBox"),
        oldClassName = Y.ClassNameManager.getClassName('tooltip', 'position', e.prevVal),
        newClassName = Y.ClassNameManager.getClassName('tooltip', 'position', e.newVal);

        bb.replaceClass(oldClassName, newClassName);
    },
    ...
    /*
     * Attaches the default mouseover and focus DOM listener to the current delegate node
     */
    ...
    _bindDelegate : function() {
        ...
        eventHandles.delegate = Y.delegate(["mouseenter", "focus"], Y.bind(this._onNodeEventEnter, this), this.get("delegate"), "." + this._triggerClassName);
    },

    /*
     * Default event enter (mouseenter, focus) DOM event listener.
     *
     * Delegates to the _enterTrigger method,
     * if the mouseover/focus enters a trigger node.
     */

    _onNodeEventEnter : function(e) {
    ...
    /*
     * Default blur DOM event listener
     *
     * Delegates to _leaveTrigger
     */

    _onNodeBlur : function(e) {
        this._leaveTrigger(e.currentTarget);
    },

    /*
     * Default handler invoked when the mouse enters or focuses on
    ...
    _showTooltip : function() {
        ...
        this.align(this._currTrigger.node, {
            north: [Y.WidgetPositionAlign.BC, Y.WidgetPositionAlign.TC],
            east: [Y.WidgetPositionAlign.LC, Y.WidgetPositionAlign.RC],
            south: [Y.WidgetPositionAlign.TC, Y.WidgetPositionAlign.BC],
            west: [Y.WidgetPositionAlign.RC, Y.WidgetPositionAlign.LC]
        }[this.get("compass")]);
    ...
    */
    ...
    _setTriggerContent : function(node) {
        var content = this.get("content") || true;
    ...
    _setCurrentTrigger : function(node, x, y) {
         ...
         triggerHandles.blur = Y.on("blur", Y.bind(this._onNodeBlur, this), node);
    ...
    _clearHandles : function() {
        ...
        if (eventHandles.trigger.blur) {
            eventHandles.trigger.blur.detach();
        }
    }
}, {
    ...
    ATTRS : {
        ...
        /*
         * The compass based location of the tooltip relative to the trigger
         * node.
         */
        compass: {
            value: 'south'
        ...
        /*
         * Whether to escape the string used for the tooltip. Defaults to true.
         */
        escapeHTML: {
            value: true
        }
...
Y.namespace("U1.Widget");
Y.U1.Widget.Tooltip = Tooltip;
