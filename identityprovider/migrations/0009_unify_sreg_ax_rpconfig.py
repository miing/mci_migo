...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Merge OpenIDRPConfig.allowed_ax and OpenIDRPConfg.allowed_sreg
        # So we take the value of both columns, create a set and populate the
        # new column.
        rpconfig_model = orm['identityprovider.OpenIDRPConfig']
        rpconfigs = rpconfig_model.objects.filter(
            Q(allowed_ax__isnull=False) | Q(allowed_sreg__isnull=False))
        for rpconfig in rpconfigs:
            allowed_ax = rpconfig.allowed_ax
            allowed_ax = allowed_ax.split(',') if allowed_ax else []
            allowed_sreg = rpconfig.allowed_sreg
            allowed_sreg = allowed_sreg.split(',') if allowed_sreg else []
            allowed_user_attribs = set(allowed_ax + allowed_sreg)
            rpconfig.allowed_user_attribs = ','.join(allowed_user_attribs)
            rpconfig.save()

    def backwards(self, orm):
        "Write your backwards methods here."
        # So we take the value of the old column and copy it into the two new
        # columns, taking care to filter account_verified from allowed_sreg.
        rpconfig_model = orm['identityprovider.OpenIDRPConfig']
        rpconfigs = rpconfig_model.objects.filter(
            allowed_user_attribs__isnull=False)
        for rpconfig in rpconfigs:
            allowed_user_attribs = rpconfig.allowed_user_attribs
            allowed_user_attribs = (
                allowed_user_attribs.split(',') if allowed_user_attribs
                else [])
            allowed_sreg = [i for i in allowed_user_attribs
                            if i != 'account_verified']
            rpconfig.allowed_sreg = ','.join(allowed_sreg)
            rpconfig.allowed_ax = ','.join(allowed_user_attribs)
            rpconfig.save()
    ...
    models = {
        'identityprovider.account': {
            ...
            'openid_identifier': ('django.db.models.fields.TextField', [], {'default': "u'PNywzmm'", 'unique': 'True'}),
