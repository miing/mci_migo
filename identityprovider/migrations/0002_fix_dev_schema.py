...
class Migration(SchemaMigration):
    ...
    sql_files = [
        'generate_openid_identifier.sql',
        'emailaddress_indexes.sql',
        'emailaddress_constraints.sql',
        'openidauthorization_indexes.sql',
    ]

    def forwards(self, orm):
        basedir = os.path.abspath(os.path.dirname(__file__))
        for filename in self.sql_files:
            with file(os.path.join(basedir, 'sql', filename)) as sql_file:
                sql = sql_file.read()
                db.execute(sql)
    ...
    models = {
        'identityprovider.account': {
            ...
            'openid_identifier': ('django.db.models.fields.TextField', [], {'default': "u'wQmPtKM'", 'unique': 'True'}),
