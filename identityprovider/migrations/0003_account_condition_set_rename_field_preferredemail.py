...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for switch in orm['gargoyle.Switch'].objects.all():
            value = switch.value
            if 'identityprovider.account' in value:
                condition = value['identityprovider.account']
                if 'preferredemail' in condition:
                    condition['email'] = condition['preferredemail']
                    del condition['preferredemail']
                    switch.value = value
                    switch.save()
        ...
        for switch in orm['gargoyle.Switch'].objects.all():
            ...
            if 'identityprovider.account' in value:
                ...
                if 'email' in condition:
                    condition['preferredemail'] = condition['email']
                    del condition['email']
                    switch.value = value
                    switch.save()
    ...
    models = {
        ...
        'identityprovider.account': {
            ...
            'openid_identifier': ('django.db.models.fields.TextField', [], {'default': "u'86DnfJP'", 'unique': 'True'}),
    ...
    complete_apps = ['gargoyle', 'identityprovider']
    symmetrical = True
