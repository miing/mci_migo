...
class Migration(DataMigration):
    # original value of EmailStatus.INVALIDATED
    INVALIDATED_EMAIL_STATUS = 5

    def _get_account_from_person_id(self, orm, person_id):
        # needed to add this because of South freezing models in orm
        person_model = orm['identityprovider.Person']
        try:
            person = person_model.objects.get(id=person_id)
        except person_model.DoesNotExist:
            person = None

        lp_id_model = orm['identityprovider.LPOpenIdIdentifier']
        account_model = orm['identityprovider.Account']
        if person is not None and person.lp_account is not None:
            open_ids = lp_id_model.objects.filter(
                lp_account=person.lp_account)
            if len(open_ids) > 0:
                openid_identifier = open_ids[0].identifier
                accounts = account_model.objects.filter(
                    openid_identifier=openid_identifier)
                if len(accounts) > 0:
                    return accounts[0]

    def forwards(self, orm):
        "Migrate invalidated email address to new table."
        emailaddress_model = orm['identityprovider.EmailAddress']
        invalidatedemail_model = orm['identityprovider.InvalidatedEmailAddress']
        
        invalidated_emails = emailaddress_model.objects.filter(
            status=self.INVALIDATED_EMAIL_STATUS)

        for email in invalidated_emails:
            account = email.account
            if account is None and email.lp_person:
                account = self._get_account_from_person_id(orm, email.lp_person)

            # if there is no associated account, skip (but log the details)
            if account:
                invalidatedemail_model.objects.create(
                    email=email.email, account=account,
                    date_created=email.date_created)
                email.delete()
            else:
                logging.warning(
                    "[INVALIDATED EMAILS MIGRATION] "
                    "Could not create invalidated entry for (%s, %s), "
                    "no associated account found", email.id, email.email)

    def backwards(self, orm):
        "Migrate invalidated email address back to emails table."
        emailaddress_model = orm['identityprovider.EmailAddress']
        invalidatedemail_model = orm['identityprovider.InvalidatedEmailAddress']

        invalidated_emails = invalidatedemail_model.objects.all()
        for email in invalidated_emails:
            # check if this email was re-added to valid emails table
            try:
                existent_email = emailaddress_model.objects.get(
                    email__iexact=email.email)
            except emailaddress_model.DoesNotExist:
                existent_email = None

            if (existent_email is not None and
                    existent_email.status == EmailStatus.NEW and
                    existent_email.account == email.account):
                # it already exists as NEW for the same account => invalidate
                existent_email.status = self.INVALIDATED_EMAIL_STATUS
                existent_email.save()
            elif existent_email is None:
                emailaddress_model.objects.create(
                    email=email.email,
                    status=self.INVALIDATED_EMAIL_STATUS,
                    date_created=email.date_created,
                    account=email.account,
                )
            # else: already exists but validated (ie status != NEW),
            #       or related to someone else => skip
    ...
    models = {
        'identityprovider.account': {
            ...
            'openid_identifier': ('django.db.models.fields.TextField', [], {'default': "u'wpCdtBR'", 'unique': 'True'}),
